...
// 타임어택 캐러셀 변경
function timeCarouselChange(__isChange) {

	if (__isChange === false) return;

	var data = mm.data.get(this).carousel;
	mm.observer.dispatch('TIME_ITEM_CHANGE', { _isLocal: true, data: { $item: data.__.$items[data._index], _index: data._index } });
...
(function () {
	...
	function setTime() {
		...
		_.forEach($timers, function (__$timer) {
			...
			if (_time === 0) {
				...
				if (!$a) {
					// 플립
					if (_isFlip) $a = mm.find('.mm_carousel-item a', mm.siblings(__$timer, '.mm_carousel'))[0];
					// 딜 목록
					else {
						_isListDeal = true;
	...
	mm.observer.on(_.filter($timers, function (__$timer) { return __$timer.classList.contains(_classFlip) }), 'TIME_ITEM_CHANGE', function (__e) {
		...
		mm.element.attribute(mm.siblings($item.closest('.mm_carousel'), '.m_timedeal-timer'), { 'data-value': data });
		setTime();
