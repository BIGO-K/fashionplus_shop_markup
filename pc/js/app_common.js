...
mm.switch = (function () {
	...
	var initial = {
		...
		_classParent: null,// ? :string - _classOn을 적용할 closest 요소
		_isReturnParams: true,// ? :boolean - onChange의 1번째 인자로 true/false 전달 여부
		onChange: null,// ? :function
		onChangeParams: [],// ? :array - onChange의 2번째 이후 아규먼트로 전달
	};
	// initial._isParent 값이 true이고, initial._classParent 값이 없으면 부모 요소에 적용
	...
	var base = {
		...
		toggle: function (__$switch, __is) {
			...
			var $switch = (data._isParent !== true) ? __$switch : (typeof(data._classParent) === 'string') ? __$switch.closest(mm.selector(data._classParent, '.')) : __$switch.parentElement;
...
mm.datepicker = (function () {
	...
	var initial = {
		...
		onSelect: null, // 날짜선택 callback
		onSelectArgs: [],
		__: { // 내부사용
			_pickDate: null, // 달력에서 선택한 날짜
			_periodIndex: null, // 기간 input의 index
			_periodStart: null, // 기간 시작일
			_periodEnd: null, // 기간 종료일
			onSelect: function () {}, // 날짜 선택 함수
			onDraw: function () {}, // 캘린더 이동 함수
			onOpen: function () {} // 데이트피커 오픈
		}
	};
	...
	function getDatepickerElement(__element) {
		...
		return ($el[0] && $el[0].hasAttribute(_dataName)) ? $el : $el.find(mm.string.template('[${A}]', { A: _dataName }));
	};

	// Date parameter 유효성 검사
	// date와 format을 parameter로 전달해야 합니다
	// date의 형식이 format의 형식과 다른경우에는 false를 리턴하고, format과 형식은 같지만 new Date()에서 arguments로 사용할 수 없는 형식의
	// date는 new Date() 에서 사용할 수 있는 형식으로 변환하여 그 변환값을 리턴합니다.
	function dateFormatValidate(__date, __data) {

		if (!isNaN(new Date(__date))) return changeFormat(__date);
		var _format = __data.configs._format;

		var dateStringArr = [];
		var dateSeparator = []; // date의 구분자
		var formatSeparator = []; // 포맷의 구분자

		var _arrIndex = 0;
		_.each(__date.split(''), function (__item) {

			if ((/[0-9]/g).test(__item)) dateStringArr[_arrIndex] = (!dateStringArr[_arrIndex]) ? __item : dateStringArr[_arrIndex] + __item;
			else {
				dateSeparator.push(__item);
				if (dateStringArr[_arrIndex]) _arrIndex++;
			}

		});

		_.each(_format.split(''), function (__item){
			if (!(/(Y|M|D)/g).test(__item)) formatSeparator.push(__item);
		})

		if (formatSeparator.join('.') != dateSeparator.join('.')) {
			console.error('datepicker의 날짜 형식이 포맷과 일치하지 않습니다.');
			...
			_format.replace(/(YYYY|YY|MM|M|DD|D)/gi, function (__type) {

				if ((/(YYYY)/).test(__type)) result[0] = dateStringArr[_arrIndex];
				else if ((/(YY)/).test(__type)) result[0] = mm.string.template(['${A}${B}'], { A: Math.floor(new Date().getFullYear() / 100), B: dateStringArr[_arrIndex] });
				else if ((/(MM|M)/).test(__type)) result[1] = dateStringArr[_arrIndex];
				else if ((/(DD|D)/).test(__type)) result[2] = dateStringArr[_arrIndex];
				_arrIndex++;

			});
		}
		return changeFormat(result.join('/'));

	};

	// 날짜의 포맷 형식 변경
	// date, format, 월선택 여부를 parameter로 전달하며,
	// 변경하고자 하는 date의 값은 항상 전달해야 하고, format이 지정되지 않으면 YYYY-MM-DD형식으로 리턴합니다
	// 월 선택을 전달하는 경우 format에 DD가 있어도 DD는 제거하고 YYYY, MM만 리턴합니다.
	function changeFormat(__date, __data) {

		var date = (__date) ? new Date(__date) : new Date();
		var _format = (__data) ? __data.configs._format : 'YYYY-MM-DD';
		var _isMonth = (__data) ? __data.configs._isSelectMonth : false;

		// 월 선택일 경우 포맷 처리(포맷에서 DD제거)
		if (_isMonth && (/(DD|D)/).test(_format)) {
			var format = [];
			var formatTypes = [];
			var formatSeparator = [];

			var _arrIndex = 0;
			_.each(_format.split(''), function (__type, __index) {

				if ((/(Y|M|D)/g).test(__type)) formatTypes[_arrIndex] = (!formatTypes[_arrIndex]) ? __type : formatTypes[_arrIndex] + __type;
				else {
					if (formatTypes[_arrIndex]) _arrIndex++;
					formatSeparator.push(__type);
				}

			});

			// 구분자 처리
			var chunk = _.chunk(formatSeparator, Math.ceil(formatSeparator.length / formatTypes.length));
			_.each(formatTypes, function (__type, __index) {

				if (__type.indexOf('DD') >= 0) return;

				if (chunk.length < 3 && __index != 0) format.push(__type);
				else format.push(_.concat(__type, chunk[__index]).join(''));
		...
		var stringDate = _format.replace(/(YYYY|YY|MM|M|DD|D)/gi, function (__type) {
			switch (__type) {
	...
	// 문자열 또는 객체형태의 date를 전달받아 [ object 타입 date, String 타입 date ] 배열로 리턴합니다.
	// date를 paramerter로 전달하지 않을 경우 오늘 날짜를 기준으로 처리합니다.
	function getDate(__date) {

		var date = (__date) ? new Date(__date) : new Date();
		var stringDate = changeFormat(date);

		return [date, stringDate]; // [ object 타입 date, String 타입 date ] 배열을 리턴합니다

	};

	// datepicker 생성
	// datas는 항상 전달 받아야 하며, date 값을 전달받으면 date 값에 해당하는 월을 기준으로 캘린더를 생성합니다.
	// datepicker가 생성된 후 생성된 datepicker를 리턴합니다.
	function createDatepicker(__datas, __date, __element) {

		var $datepicker = ($(__element)[0]) ? $(__element) : $('<div class="mm__datepicker"></div>');
		var date = getDate(__date)[0];

		// datepicker html구조 생성
		if (!__datas.__._isOpen) {
			if (__datas.configs._isInline) $datepicker.addClass('__datepicker-inline');
			else $datepicker.appendTo('body');

			var _itemLength = __datas.configs._isSelectMonth ? 1 : __datas.configs._multipleMonth;

			for (var _i = 1; _i <= _itemLength; _i++) {
				var $template = $(mm.string.template([
				...
				var $thead = $template.find('thead')[0].insertRow();

				if (__datas.configs._isSelectMonth) $('<th scope="col" colspan="4">월을 선택하세요</th>').appendTo($thead);
				else {
					// 월선택 셀렉트을 추가하기 위해 클론
					$template.find('.mm__datepicker__head-label').after($template.find('.mm__datepicker__head-label').clone());

					// 요일(일~토) 항목 추가
					for (var _weekIndex = 0; _weekIndex < 7; _weekIndex++) {
						$(mm.string.template(['<th scope="col">${A}</th>'], { A: __datas.configs.weekdays[_weekIndex] })).appendTo($thead);
					}
				}
			}

			var _today = getDate()[1];
			if (__datas.configs._isSelectMonth) _today = _today.slice(0,7);

			// $(mm.string.template([
			// 	'<div class="mm__datepicker-foot">',
			// 	'	<button type="button" class="mm_btn btn_today __btn_sm_line_lighter__" data-datepicker-date="${A}"><b>오늘로 설정</b></button>',
			// 	'</div>',
			// ], { A: _today }).replace(/\n/g, "")).appendTo($datepicker)
			// .find('.btn_today').off('click', __datas.__.onSelect).on('click', __datas.__.onSelect);

			if(__datas.__._periodEnd || __datas.__._periodStart) {
				if (__datas.__._periodEnd <= _today && __datas.__._periodIndex === 0) $('.btn_today').attr('disabled', true);
				if ( __datas.__._periodStart >= _today && __datas.__._periodIndex === 1) $('.btn_today').attr('disabled', true);
			}

			// var $controls = $(mm.string.template([
			...
			// if (__datas.configs._isSelectMonth) $controls.splice(0, 2);
			// $controls.appendTo($datepicker).off('click', __datas.__.onDraw).on('click', __datas.__.onDraw);
		}

		_.each($datepicker.find('.mm__datepicker-item'), function (__item, __index) {

			var $item = $(__item);
			var _year = date.getFullYear();
			var _month = date.getMonth() + 1;

			if (__datas.__._isOpen) $item.find('tbody').empty();

			_.each($item.find('.mm__datepicker__head-label'), function (__label, __index) {

				var $select = $(__label).find('select');
				$select.empty();
				...
				for (var _i = _min; _i <= _max; _i++) {
					var $options = $(mm.string.template('<option value="${A}">${A}</option>', { A: _i })).appendTo($select);
				...
				$select.off().on('change', __datas.__.onDraw);
			...
			var startDate = null; // 캘린더 시작일자
			var endDate = null; // 캘린더 마지막일자
			var currentDate = null;

			if (__datas.configs._isSelectMonth) {
				startDate = 1;
				endDate = 12;
				currentDate = startDate;

				while (currentDate <= endDate) {
					dateArray.push(mm.string.template('${A}-${B}-01', { A: _year, B: String(currentDate).padStart(2, '0') }));
					currentDate++;
				}
			}
			else {
				var _startIndex = -__datas.configs._firstDay + new Date(_year, _month - 1, 1).getDay();
				var _endIndex = new Date(_year, _month, 0).getDay() - __datas.configs._firstDay;
			...
			dateArray.forEach(function (__objDate, __index) {

				var $btnDate = null;
				var dateItem = getDate(__objDate);

				// 달력 날짜 생성
				if (__datas.configs._isSelectMonth) {
					if (__index % 4 === 0) $row = $item.find('tbody')[0].insertRow();

					$btnDate = $(mm.string.template('<button type="button" class="btn_month" data-datepicker-date="${A}"><b>${B}</b></button>', {
						A: dateItem[1], B: dateItem[0].getMonth() + 1
					}));
				}
				else {
					if (__index % 7 === 0) $row = $item.find('tbody')[0].insertRow();

					$btnDate = $(mm.string.template('<button type="button" class="btn_date" data-datepicker-date="${A}"><b>${B}</b></button>', {
						A: dateItem[1], B: dateItem[0].getDate()
					}));
				}

				var $cell = $($row.insertCell());
				$btnDate.appendTo($cell).off('click', __datas.__.onSelect).on('click', __datas.__.onSelect);


				// 날짜에 해당하는 클래스 추가(이전달, 다음달, 기간, 선택된 날짜 등)
				if (!__datas.configs._isSelectMonth && dateItem[0].getMonth() + 1 != _month) $cell.addClass('__datepicker-outside-month');

				if (__datas.__._periodStart || __datas.__._periodEnd) {
					$btnDate.prop('disabled', function () {
						return (__datas.__._periodIndex === 1 && __datas.__._periodStart > dateItem[1] || __datas.__._periodIndex === 0 && __datas.__._periodEnd < dateItem[1]);
					});

					if (__datas.__._periodStart === dateItem[1] || __datas.__._periodEnd === dateItem[1]) $cell.addClass('__datepicker-selected');
					else if (__datas.__._periodStart < dateItem[1] && __datas.__._periodEnd > dateItem[1] ) $cell.addClass('__datepicker-period');
				}
				else if (dateItem[1] === __datas.__._pickDate) $cell.addClass('__datepicker-selected');

				// 특정일(_disableBeforeDate, _disableAfterDate) 전후의 날짜는 disabled
				if (__datas.configs._disableBeforeDate || __datas.configs._disableAfterDate) {

					var _disableBefore = (__datas.configs._disableBeforeDate) ? dateFormatValidate(__datas.configs._disableBeforeDate, __datas.configs._format) : null;
					var _disableAfter = (__datas.configs._disableAfterDate) ?  dateFormatValidate(__datas.configs._disableAfterDate, __datas.configs._format) : null;

					$btnDate.prop('disabled', function() {
						return (_disableBefore > dateItem[1] || _disableAfter < dateItem[1]);
					});
				}
			...
			// date의 날짜를 1일로 바꾸더라도 현재 선택된 일자는 __datas.__._pickDate로 가져오기 때문에 동작에 영향은 없습니다.
	...
	return {
		update: function (__element) {
			...
			var $element = getDatepickerElement(__element);
			$element.each(function () {

				var $ui = $(this);
				var $formText = $ui.closest('.mm_form-text');
				var data = mm.data.get($ui, _dataName, { initial: initial });
				if (mm.is.empty(data)) data = mm.data.set($ui, _dataName, { initial: initial });

				// 인풋 키입력 안되도록 적용
				$formText.addClass('__text_calendar__');

				if ($ui.prop('readonly')) $formText.find('.btn_text-clear').remove();

				// 요일 시작일에 따른 요일 배열 정렬
				data.configs.weekdays = data.configs.weekdays.concat(data.configs.weekdays.slice(0, data.configs._firstDay)).splice(data.configs._firstDay);

				// value가 있을 경우 value값이 포맷 형식인지 유효성 체크
				if ($ui.val()) {
					var _value = dateFormatValidate($ui.val(), data);

					if (_value) data.__._pickDate = _value;
					else $ui.val(String(_value));
				}
				...
				data.__.onSelect = function (__e) {
					...
					if (data.configs._isInline) data.__.onClose();

					var $item = $(__e.target).closest('button');
					var $datepicker = $item.closest('.mm__datepicker');
					var _pickDate = $item.data('datepickerDate'); // 캘린더의 버튼 요소가 가지고 있는 YYYY/MM/DD 형식의 data 값

					if (data.configs._isInline && $item.is('.btn_today')) createDatepicker(data, _pickDate, $datepicker);
					else {
						$ui.val(changeFormat(_pickDate, data));
						mm.form.update();
					}

					$datepicker.find('.__datepicker-selected').removeClass('__datepicker-selected').end()
					.find(mm.string.template('[data-datepicker-date="${A}"]', { A: _pickDate })).closest('td').addClass('__datepicker-selected');

					data.__._pickDate = _pickDate;
					data.onSelectArgs = getDate(_pickDate);

					if (data.configs._isSelectMonth) data.onSelectArgs[1] = data.onSelectArgs[1].slice(0, 7);

					mm.apply(data.onSelect, $ui, data.onSelectArgs);
					mm.delay.on(function () {
				...
				data.__.onDraw = function (__e) {
					...
					var $datepickerHead = $datepicker.find('.mm__datepicker-item-head');
					var changeDate = null;
					...
					switch (__e.type) {
						...
						case 'change':
							...
							changeDate = $calendarHead.find('select')[0].value;

							if (!data.configs._isSelectMonth) {
								var _month = $calendarHead.find('select')[1].value;
								changeDate += mm.string.template('-${A}', { A: String(_month).padStart(2, '0') });
							}

							changeDate = getDate(changeDate)[0];
							changeDate.setMonth(changeDate.getMonth() - $datepickerHead.index($calendarHead));
							break;
						// 버튼
						case 'click':
							changeDate = $datepickerHead.find('select')[0].value;

							if (!data.configs._isSelectMonth) {
								var _month = $datepickerHead.find('select')[1].value;
								changeDate += mm.string.template('-${A}', { A: String(_month).padStart(2, '0') });
							}

							changeDate = getDate(changeDate)[0];
							if ($this.is('.btn_next-month')) changeDate.setMonth(changeDate.getMonth() + 1);
							else if ($this.is('.btn_prev-month')) changeDate.setMonth(changeDate.getMonth() - 1);
							else if ($this.is('.btn_next-year')) changeDate.setFullYear(changeDate.getFullYear() + 1);
							else if ($this.is('.btn_prev-year')) changeDate.setFullYear(changeDate.getFullYear() - 1);
							break;
					}

					createDatepicker(data, changeDate, $datepicker);

				};

				// 데이트픽커 오픈
				data.__.onOpen = function (__e) {

					__e.stopPropagation();

					if ($('.mm__datepicker').not('.__datepicker-inline').data('target') === __e.target) return;
					data.__.onClose();

					// 기간
					var $period = $ui.closest('.mm_formmix-period');
					if ($period[0]) {
						var $periodEl = $period.find(mm.string.template('input[${A}]', { A: _dataName }));

						data.__._periodIndex = $periodEl.index($ui);
						data.__._periodStart = $periodEl[0].value ? mm.data.get($periodEl.eq(0), _dataName).__._pickDate : null;
						if ($periodEl.length > 1) data.__._periodEnd = $periodEl[1].value ? mm.data.get($periodEl.eq(1), _dataName).__._pickDate : null;
					}

					var $datepicker = createDatepicker(data, data.__._pickDate);
					$datepicker.css({ 'top': $formText.offset().top + $formText.height() + 8, 'left': $ui.offset().left })
					.data('target', __e.target);

					data.__._isOpen = true;

					$(document).on('click', function closeDatepicker(__e) {

						var $target = $(__e.target);

						if ($target.closest('.mm__datepicker')[0] || (data.configs._isInline && $target.closest('.__datepicker-inline')[0])) return;
						data.__.onClose();

						$(this).off('click', closeDatepicker);

					});

					$ui.on('keyup', function typingDate(__e) {

						if (__e.keyCode === 13) {
							if (data.__._pickDate === getDate($ui.val())[1]) {
								$ui.off('keyup', typingDate);
							...
							if (_value && _value != "NaN-NaN-NaN") {
								data.__._pickDate = _value;

								if (__e.keyCode === 13) createDatepicker(data, data.__._pickDate, $datepicker);
							...
							else console.error('datepicker의 날짜 형식이 올바르지 않습니다.');
						}
				...
				// 데이트픽커 닫기
				data.__.onClose = function () {

					$('.mm__datepicker').not('.__datepicker-inline').remove();
					if (!data.configs._isInline) data.__._isOpen = false;

				}

				if (data.configs._isInline && !data.__._isOpen) {
					var $datepicker = createDatepicker(data);
					$datepicker.addClass('__datepicker-inline').appendTo($ui);
					data.__._isOpen = true;
				}
				else $ui.off('click', data.__.onOpen).on('click', data.__.onOpen);

			});
		},
		// date 변경 (외부에서 datepicker 값 변경 element 와 date 값을 필수값으로 전달해야 합니다.)
		change: function (__element, __date) {
			// ? __date:string - 변경할 날짜

			var $el = getDatepickerElement(__element);

			if (!$el[0] || !__date) return false;

			var data = mm.data.get($el, _dataName);
			var _changeValue = dateFormatValidate(__date, data);

			if (_changeValue) {
				$el.val(changeFormat(_changeValue, data));
				data.__._pickDate = (data.configs._isSelectMonth) ? _changeValue.slice(0,7) : _changeValue;

				mm.form.update();
			}

		},
		// datepicker 인풋 비우기 처리
		clear: function (__element) {

			__element.off('click');
			__element.removeData();

			mm.datepicker.update(__element);
